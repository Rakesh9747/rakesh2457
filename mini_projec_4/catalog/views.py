...
@login_required
def home(request):
    products = Product.objects.all()[:4]  # Show 4 featured products
...
def download_invoice(request):
    ...
    p.drawString(400, 800, f"Date: {date_str}")
...
@login_required
def credit_card_entry(request):
    if request.method == 'POST':
        form = CreditCardForm(request.POST)
        if form.is_valid():
            form.save()
            return render(request, 'waiting_payment.html')
    else:
        form = CreditCardForm()
...
def place_order(request):
    ...
    if not cart_items.exists():
        return redirect('product_list')  # Or show a message: "Cart is empty"
